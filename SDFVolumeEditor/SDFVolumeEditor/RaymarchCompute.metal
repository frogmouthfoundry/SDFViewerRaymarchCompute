...
constant float3 BASE_COLOR = float3(0.8, 0.2, 0.1);
...
void raymarchKernel(uint2 gid [[thread_position_in_grid]],
    ...
    // Get camera and volume parameters from explicit fields
    float3 camPos = float3(params.cameraPositionX, params.cameraPositionY, params.cameraPositionZ);
    float3 camFwd = float3(params.cameraForwardX, params.cameraForwardY, params.cameraForwardZ);
    float3 camRight = float3(params.cameraRightX, params.cameraRightY, params.cameraRightZ);
    float3 camUp = float3(params.cameraUpX, params.cameraUpY, params.cameraUpZ);
    float3 volMin = float3(params.volumeMinX, params.volumeMinY, params.volumeMinZ);
    float3 volMax = float3(params.volumeMaxX, params.volumeMaxY, params.volumeMaxZ);
    uint3 volDims = uint3(params.volumeDimX, params.volumeDimY, params.volumeDimZ);
